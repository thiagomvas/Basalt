...
entity.Transform.Position = new(0, 1, 0);
entity.Transform.Rotation = Quaternion.CreateFromYawPitchRoll(1, 1, 0);
var clone = entity.Clone();

foreach(var prop in entity.Transform.GetType().GetProperties())
{
	Console.WriteLine($"{prop.Name}: {prop.GetValue(entity.Transform)}");

}

foreach (var prop in clone.Transform.GetType().GetProperties())
{
	Console.WriteLine($"Cloned {prop.Name}: {prop.GetValue(clone.Transform)}");
}
